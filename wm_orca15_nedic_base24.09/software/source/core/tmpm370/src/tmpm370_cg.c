...
Result CG_SetPLL(FunctionalState NewState)
{
    ...
    {
        fcsrc = CG_GetFcSrc();
        if (fcsrc == CG_FC_SRC_FOSC)
        { /* PLL is not used. So it can be shut off */
            /* Set register to disable PLL */
            TSB_CG_OSCCR_PLLON = 0U;
            retval = SUCCESS;
        }
        else
        { /*fcsrc == CG_FC_SRC_FPLL. PLL is in use, so disabling it should be invalid */
            /* Do nothing */
        }
    }
...
#if defined(__TMPM372_CG_H) || defined(__TMPM373_CG_H) || defined(__TMPM374_CG_H)
    tmp = CG_GetFoscState(CG_FOSC_OSC2);
    if ((CG_GetFoscState(CG_FOSC_OSC1) == ENABLE) || (tmp == ENABLE))
    {
        fosc_on = ENABLE;
    }
    else
